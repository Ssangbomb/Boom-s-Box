<?php
  ...
  while($row = mysqli_fetch_array($result)){
    $list = $list."<li><a href=\"editcustomer.php?id={$row['id']}\">{$row['first_name']} {$row['last_name']}</a></li>";
  }
  $info = array(
    'first_name' => '',
    'last_name' => '',
  ...
  if(isset($_GET['id'])){
    ...
    $query = "SELECT * FROM invoice WHERE id={$filtered_id}";
    $result = mysqli_query($conn, $query);
...
<html>
  ...
  <body>
    ...
    <form action="customeredited.php" method="post">
